...
from preprocess1 import get_selected_points2d, get_camera_intrinsics
from preprocess1 import SCENE_GRAPH_FILE, RANSAC_MATCH_DIR, RANSAC_ESSENTIAL_DIR, HAS_BUNDLE_ADJUSTMENT, RESULT_DIR
from bundle_adjustment1 import compute_ba_residuals
...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    Returns:
        [image_id1, image_id2]
    """
    max_pair = [None, None]  # 初始化为无效值
    """ YOUR CODE HERE """
    max_num_inliers = 0 # 当前最大内点数
    # 遍历场景图中的每个图像
    for image_id1 in scene_graph.keys():
        neighbors = scene_graph[image_id1]# 获取邻居图像
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    # 在 correspondences2d3d 中添加新的 2D 到 3D 的对应关系
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    for registered_id in registered_ids:
        neighbors = scene_graph[registered_id]# 获取邻近的图像
        for new_id in neighbors:
            if new_id not in registered_ids:# 只处理未注册的图像
                matches = load_matches(registered_id, new_id)# 加载匹配点
                num_inliers = matches.shape[0]# 计算入射点数
                # 如果当前入射点数大于最大入射点数，则更新
                if num_inliers > max_num_inliers:
                    max_num_inliers = num_inliers
                    max_new_id = new_id
