...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    # 计算投影矩阵
    selected_extrinsics = extrinsics[camera_idxs]# 选择相应的外参
    P = np.matmul(intrinsics, selected_extrinsics)# C x 3 x 4

    # 重投影到2D平面
    calculated_points2d = np.einsum('ijk,ki->ij', P, homo_3d_points_T)# 计算重投影
    calculated_points2d /= calculated_points2d[:, -1].reshape((calculated_points2d.shape[0], 1))# 齐次化结果
    calculated_points2d = calculated_points2d[:, :-1]# 只保留前两维

    # 计算残差
    residuals = np.linalg.norm(points2d - calculated_points2d, axis=1)# 欧几里得距离
    """ END YOUR CODE HERE """
    return residuals
