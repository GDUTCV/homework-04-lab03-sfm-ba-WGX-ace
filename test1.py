...
def check_folder(directory, func):
    ...
    for file in files:
        ...
        # 确保比较两个数组的形状相同
        if ta.shape == prd.shape:
            num_match += np.all(ta == prd)
        else:
            print(f"Shape mismatch for {file}: {ta.shape} vs {prd.shape}")
...
def check_npy_file(file_path, func):
    ...
    print(f"Shape of ta: {ta.shape}, Shape of prd: {prd.shape}")  # 输出形状

    if ta.shape != prd.shape:
        print(f"Error: Shape mismatch! ta shape: {ta.shape}, prd shape: {prd.shape}")
        return  # 停止比较，或根据需要处理
